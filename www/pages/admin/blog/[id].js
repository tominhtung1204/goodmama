...
const EditBlog = props => {
  ...
  useEffect(() => {
    ...
    getBlogById(router.query.id).then(data => {
      setControls(data);
    });
  }, []);
  ...
  const updatedBlog = value => {
    ...
    setLoaded(true);
    updateBlog(router.query.id, body)
      .then(data => {
  ...
  return (
    <Row>
      <Col xs={12}>
        <Card>
          <Card.Header>
            <h5 className="card-title">Chỉnh sửa bài viết</h5>
            <h6 className="card-subtitle text-muted">Chỉnh sửa bài viết</h6>
          </Card.Header>
          <Card.Body>
            <Formik
              initialValues={{
