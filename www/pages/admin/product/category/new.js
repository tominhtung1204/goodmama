...
import { createCategoryProduct } from "../../../../services/actions";
...
const AddCategoryProduct = props => {
  ...
  const createdCategory = value => {
    const body = new FormData();
    body.append("title", value.title);
    body.append("description", value.description);
    body.append("children", JSON.stringify(controls.children));
  ...
  return (
    <Row>
      <Col xs={12}>
        <Card>
          <Card.Header>
            <h5 className="card-title">Thêm mới danh mục sản phẩm</h5>
...
export default withAuth(AddCategoryProduct);
