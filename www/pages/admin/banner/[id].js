...
import { updateBanner, getBannerById } from "../../../services/actions";
...
const EditBanner = props => {
  ...
  useEffect(() => {
    getBannerById(router.query.id).then(res => {
      setControls(res);
    });
  }, []);
  ...
  const updatedBanner = (id, value) => {
    ...
    updateBanner(id, body)
      .then(data => {
        toast.success("Updated banner thành công");
        setLoaded(false);
        router.push("/admin/banner");
      })
      .catch(err => {
        setLoaded(false);
        toast.error("Không thể update banner");
      });
  };
  ...
  return (
    <Row>
      <Col xs={12}>
        <Card>
          <Card.Header>
            <h5 className="card-title">Chỉnh sửa banner</h5>
            <h6 className="card-subtitle text-muted">
              Chỉnh sửa banner quảng cáo trang chủ
            </h6>
          </Card.Header>
          <Card.Body>
            <Formik
              ...
              onSubmit={values => {
                updatedBanner(router.query.id, values);
              }}
            >
